...
excel_path = "Бренды_Напольные_без категории_export_05.02.25.xlsx"
...
def get_description(brand):
    message = 'Напиши привлекательное описание для фабрики ' + brand + ' по производству выпускаемых фабрикой напольных покрытий'
    print(message)
...
try:
    ...
    for row in sheet.iter_rows(min_row=2, max_row=sheet.max_row, min_col=1, max_col=4):
        ...
        if brand:
            ...
            try:
                description = get_description(brand)
